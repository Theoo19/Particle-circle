...
function ParticleCircle(amount, x, y, radius, colors){
	this.amount = amount
	this.x = x
	this.y = y
	this.radius = radius
	...
	this.new = function(){
		for (let i = 0; i < this.amount; i++){
			radius = randint(this.radius - 5, this.radius + 5)
			...
			dir = randint(right_angle - 20, right_angle + 5) * (Math.PI / 180)
			dxy = randint(10, 30) / 5
			dx = dxy * Math.sin(dir)
			dy = dxy * Math.cos(dir)
...
var Circle = new ParticleCircle(20, innerWidth / 2, innerHeight / 2, 300, colors)
